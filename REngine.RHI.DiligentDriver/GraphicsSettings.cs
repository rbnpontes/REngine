...
namespace REngine.RHI.DiligentDriver
{
	...
	public class D3D12Settings
	{
		...
		private uint[] pQueryPoolSize = new uint[(int)Diligent.QueryType.NumTypes]
		{
			0,
			128,
			128,
		...
		public uint[] QueryPoolSizes
		{
			...
			set
			{
				if (value.Length != (uint)Diligent.QueryType.NumTypes)
					throw new ArgumentException($"Array must be fixed length of {(int)Diligent.QueryType.NumTypes}.");
	...
	public class VulkanSettings
	{
		...
		private uint[] pQueryPoolSizes = new uint[(int)Diligent.QueryType.NumTypes]
		{
			0,
			128,
			128,
		...
		public string[] InstanceLayerNames { get; set; } = new string[0];
		public string[] InstanceExtensionNames { get; set; } = new string[0];
		public string[] DeviceExtensionNames { get; set; } = new string[0];
		...
		public uint[] QueryPoolSizes
		{
			...
			set
			{
				if(value.Length != (int)Diligent.QueryType.NumTypes)
					throw new ArgumentException($"Array must be fixed length of {(int)Diligent.QueryType.NumTypes} items");
